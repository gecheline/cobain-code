...
class Contact_Binary(object):
    ...
    def __init__(self, mass1=1.0, q=1.0, ff=0.1, pot_range=0.01, dims=np.array([50,50,50]),
        ...
        xz, z = nekmin(self.pot, self.q, L1, 0.05)
        ...
        dimlen = dims[0] * dims[1] * dims[2]
        potlen = dims[1] * dims[2]
        breaks1 = np.zeros((dims[0], 3))
        breaks2 = np.zeros((dims[0], 3))
        ...
        for i in range(dims[0]):
            ...
            argzeros2 = np.argwhere((pot_slice2[:,0] == self.scale) & (pot_slice2[:,1] == 0.0) & (pot_slice2[:,2]==0.0))
    ...
    def critical_pots(self, q, sma=1., d=1., F=1.):
        # L1
        dxL = 1.0
        L1 = 1e-3
        while abs(dxL) > 1e-6:
            dxL = - potentials.dBinaryRochedx([L1, 0.0, 0.0], 1., q, 1.) / potentials.d2BinaryRochedx2([L1, 0.0, 0.0], 1., q, 1.)
            L1 = L1 + dxL
        Phi_L1 = potentials.BinaryRoche(np.array([L1, 0.0, 0.0]), q, 1., 1.)
    ...
    def sweep_mesh(self,points,iter_n=1):
        ...
        for j, indx in enumerate(points):
            ...
            if np.all(r==0.) or (r[0]==self.scale and r[1]==0. and r[2]==0.):
                ...
                I_new[j], thetas_new[j], phis_new[j], taus_new[j] = rt.contact_binary.run(r, self.mesh['normals'][indx],
    ...
    def plot_structure(self,type='intensity',iteration=0,direction=2):
        if type == 'intensity':
            f = np.load
