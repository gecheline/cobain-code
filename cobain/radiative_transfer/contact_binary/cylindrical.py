...
def compute_structure_points(newpoints=[],scale=1.,q=1.,interp_funcs={},pot_range=[],nekmin=0.5,r0_scales=[],stepsize=False,arg=0):
    ...
    bounds = True
    bounds_cond = np.all(xnorms_grid >= 0.) and np.all(xnorms_grid<=1.) and np.all(thetas_grid>=0.) and np.all(thetas_grid <= np.pi/2)
    if bounds_cond:
	bounds = True
    else:
	bounds = False
	...
	if bounds:
            chis[grid] = interp_funcs['chi']((pots[grid], xnorms_grid, thetas_grid))
	else:
	    cond = (xnorms_grid >= 0.) & (xnorms_grid <= 1.) & (thetas_grid >= 0.) & (thetas_grid <= np.pi/2)
	    chis[grid][cond] = interp_funcs['chi']((pots[grid][cond], xnorms_grid[cond], thetas_grid[cond]))
	    chis[grid][~cond] = np.zeros(len(pots[grid][~cond]))
	...
	if bounds:
            chis[grid] = interp_funcs['chi']((pots[grid], xnorms_grid, thetas_grid))
            Ss[grid] = interp_funcs['J']((pots[grid], xnorms_grid, thetas_grid))
            Is[grid] = interp_funcs['I'][arg]((pots[grid], xnorms_grid, thetas_grid))
	else:
	    cond = (xnorms_grid >= 0.) & (xnorms_grid <= 1.) & (thetas_grid >= 0.) & (thetas_grid <= np.pi/2)
	    chis[grid][cond] = interp_funcs['chi']((pots[grid][cond], xnorms_grid[cond], thetas_grid[cond]))
	    Ss[grid][cond] = interp_funcs['J']((pots[grid][cond], xnorms_grid[cond], thetas_grid[cond]))
            Is[grid][cond] = interp_funcs['I'][arg]((pots[grid][cond], xnorms_grid[cond], thetas_grid[cond]))
	    chis[grid][~cond] = np.zeros(len(pots[grid][~cond]))
	    Ss[grid][~cond] = np.zeros(len(pots[grid][~cond]))
	    Is[grid][~cond] = np.zeros(len(pots[grid][~cond]))
